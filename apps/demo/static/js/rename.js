function create_rename_ui() {
    ...
    html += '<h3>Rename</h3></div>';
    html += '<div class="modal-body">';
    html += '<form class="form-horizontal" method="POST" action="">';
    html += csrf_token;
    html += '<div class="control-group"><label class="control-label" for="id_new_name">New Filename</label>';
    ...
    html += '<input type="submit" class="btn btn-primary pull-right" id="rename" value="rename"/></div></div>'
    html += '</form></div>';
    $('#op-modal').html(html);
}
...
function rename(old_filename) {
    ...
    $('#new_name').blur(function() {
        ...
        if (this.value.length == 0 || this.value.length > 12) {
            ...
            $(this).parent().append('<span class="help-inline">the length of filename is not right</span>');
        } else {
...
function check_rename() {
    ...
    if (obj[0].value.length == 0 || obj[0].value.length > 12) {
        ...
        $(obj).parent().append('<span class="help-inline">the length of filename is not right</span>');
        return false;
    }
    return true;
}
